...
### 7.4 To do before opening the repo at the checkpoint

1. **Required:** confirm real names are filled into `../week3/ownership.md` (no placeholders).
2. **Required:** log a precise citation (with a real link/date) for `interest_rate_float_ref_pct` = 10.5% in `../week3/source-register.md` — currently flagged as an open item.
3. If the course has its own separate `../assessment/midterm-checklist.md` (not part of this documentation set), cross-check against that file as well before submitting — this documentation set is meant to **supplement**, not replace, the course's official checklist.
4. Confirm the `buy_closing_costs_pct` = 2.6% breakdown (registration tax + maintenance fund + notary fee) is one the team is comfortable presenting as-is, or wants to keep researching further.
